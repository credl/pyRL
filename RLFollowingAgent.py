...
class FollowingEnvironment(RLFramework.RLEnvironment):
    AC_LEFT = 0; AC_RIGHT = 1; AC_UP = 2; AC_DOWN = 3
    AC_SHOOT = 4
    
    STATE_IDX_X = 0; STATE_IDX_Y = 1; STATE_IDX_PX = 2; STATE_IDX_PY = 3
    ...
    def next(self, state, action):
        ...
        if action == self.AC_LEFT:
            ...
            if ss[self.STATE_IDX_X] < 0: ss[self.STATE_IDX_X] = 0
        elif action == self.AC_RIGHT:
            ss[self.STATE_IDX_X] += 1
            if ss[self.STATE_IDX_X] >= self.WIDTH: ss[self.STATE_IDX_X] = self.WIDTH - 1
        elif action == self.AC_UP:
            ss[self.STATE_IDX_Y] -= 1
            if ss[self.STATE_IDX_Y] < 0: ss[self.STATE_IDX_Y] = 0
        elif action == self.AC_DOWN:
            ss[self.STATE_IDX_Y] += 1
            if ss[self.STATE_IDX_Y] >= self.HEIGHT: ss[self.STATE_IDX_Y] = self.HEIGHT - 1
        # compute reward
    ...
    def visualize(self, state, rlframework):
        ...
        for y in range(50):
            for x in range(50):
                if (x, y) == (state[self.STATE_IDX_X], state[self.STATE_IDX_Y]):
                    out += "X"
                elif (x, y) == (state[self.STATE_IDX_PX], state[self.STATE_IDX_PY]):
                    out += "O"
                else:
    ...
    def __move_second_player(self, state, action):
        ...
        if state[self.STATE_IDX_PX] == 0:
            # go up at left edge
            if state[self.STATE_IDX_PY] > 0: state[self.STATE_IDX_PY] = state[self.STATE_IDX_PY] - 1
            else: state[self.STATE_IDX_PX] = state[self.STATE_IDX_PX] + 1
        elif state[self.STATE_IDX_PY] == 0:
            # go right at top edge
            if state[self.STATE_IDX_PX] < self.WIDTH - 1: state[self.STATE_IDX_PX] = state[self.STATE_IDX_PX] + 1
            else: state[self.STATE_IDX_PY] = state[self.STATE_IDX_PY] + 1
        elif state[self.STATE_IDX_PX] == self.WIDTH - 1:
            # go down at right edge
            if state[self.STATE_IDX_PY] < self.HEIGHT - 1: state[self.STATE_IDX_PY] = state[self.STATE_IDX_PY] + 1
            else: state[self.STATE_IDX_PX] = state[self.STATE_IDX_PX] - 1
        elif state[self.STATE_IDX_PY] == self.HEIGHT - 1:
            # go left at bottom edge
            if state[self.STATE_IDX_PX] > 0: state[self.STATE_IDX_PX] = state[self.STATE_IDX_PX] - 1
            else: state[self.STATE_IDX_PY] = state[self.STATE_IDX_PY] - 1
        return state
